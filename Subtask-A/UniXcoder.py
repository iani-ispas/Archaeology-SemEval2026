...
DATA_DIR = "/kaggle/input/task-a-official"
...
TRAIN_CHUNK_CHARS = 900
INFER_CHUNK_CHARS = 900
INFER_OVERLAP_CHARS = 120

MAX_CHUNKS_PER_CODE = 6
FILE_BATCH_ROWS = 512
CHUNK_BATCH_SIZE = 160
...
def basic_clean(s: str) -> str:
    ...
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    # remove markdown fence lines (train leakage)
    s = _fence_line_re.sub("", s)
    # normalize tabs (train leakage)
    s = s.replace("\t", "    ")
    # trim trailing whitespace per line
    s = "\n".join([ln.rstrip() for ln in s.split("\n")])
    # compress excessive blank lines (keep structure, reduce header noise)
    s = re.sub(r"\n{3,}", "\n\n", s)
    return s.strip()
...
def predict_proba_ai_chunked_trimmedmean_fast(
    ...
    for base in range(0, len(codes), file_batch_rows):
        ...
        # aggregate per file: trimmed mean over chunk probabilities
...
def main():
    ...
    sub_path = os.path.join(OUT_DIR, "submission_codebert_128_noval.csv")
