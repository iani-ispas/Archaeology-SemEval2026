...
class SandwichTrainCollator:
    def __init__(self, tok, max_length, mode_token, pretoken_max_chars):
        ...
        self.n_special = self.tok.num_special_tokens_to_add(pair=False)
    ...
    def __call__(self, features):
        ...
        for ex in features:
            ...
            packed = pack_head_tail_tokens(
                self.tok,
                code,
                max_body_tokens=self.max_length - self.n_special - len(self.mode_ids),
                head_frac=head_frac,
            )

            ids = self.tok.build_inputs_with_special_tokens(self.mode_ids + packed)
    ...
    def infer_collate(batch):
        ...
        for b in batch:
            ...
            packed = pack_head_tail_tokens(
                tokenizer,
                code,
                max_body_tokens=MAX_LENGTH - N_SPECIAL - len(MODE_IDS),
                head_frac=head_frac,
            )
            ids_i = tokenizer.build_inputs_with_special_tokens(MODE_IDS + packed)
...
sub_path = os.path.join(OUT_ROOT, "submission_codet5.csv")
