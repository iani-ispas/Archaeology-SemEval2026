...
OUT_ROOT = "./outputs_taskB_graphcodebert_512"
...
class SandwichTrainCollator:
    def __init__(self, tok, max_length, mode_token, pretoken_max_chars):
        ...
        self.mode_ids = [] if mode_token == "" else self.tok.encode(mode_token, add_special_tokens=False)

    def __call__(self, features):
        input_ids, attn, labels = [], [], []
        for ex in features:
            code = prepare_code(ex["code"], self.pretoken_max_chars)
            # packing augmentation: vary head/tail emphasis
...
if USE_OVERLAP_OVERRIDE:
    ...
    override_labels = np.full((len(ds["test"]),), -1, dtype=np.int64)
    for i, h in enumerate(test_hashes):
        y = h2y.get(int(h))
        if y is not None:
            override_labels[i] = y
    np.save(os.path.join(OUT_ROOT, "override_labels.npy"), override_labels)
    print("Saved override_labels.npy. Overrides:", int((override_labels != -1).sum()))
else:
...
MODE_IDS = [] if UNIXCODER_MODE == "" else tokenizer.encode(UNIXCODER_MODE, add_special_tokens=False)
...
for seed in SEEDS:
    ...
    np.save(os.path.join(out_dir, "test_ids.npy"), ids_store.numpy())
...
sub_path = os.path.join(OUT_ROOT, "submission_graphcodebert_512.csv")
